...
def create_substrate(substrate_name, **kwargs):
    ...
    if substrate_name=='boids':
        ...
        params = dict(
            ...
            border='wall',
            mix_rule='stoch',
            M=jax.numpy.array([[2, 1, 0],[0, 2, 1],[1, 0, 2]], dtype=int),
            k=9,
        )
        params.update(kwargs)
        substrate = FlowLenia(**params)
        # M= jax.numpy.array([[3, 2, 2],[2, 3, 2],[2, 2, 3]], dtype=int), k=21)
        rollout_steps = 512
        ...
        params.update(kwargs)
        substrate = ParticleLenia(**params)
