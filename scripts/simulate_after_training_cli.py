...
def main():
    parser = argparse.ArgumentParser(
        description="Legacy CLI runner for simulate_after_training without OmegaConf."
    )
    ...
    parser.add_argument('--render_mode', type=str, default='Pcolor', choices=['A', 'Pcolor', 'PcolorMix'], help='For FlowLenia: rendering mode')
    ...
    parser.add_argument('--optimize_mutation_scale', action='store_true', help='For FlowLenia: enable learned mutation scale')
    ...
    parser.add_argument('--mass_renorm', action='store_true', help='For FlowLenia: explicitly renormalize per-channel mass after RT')
    ...
    if not os.path.exists(best_path):
        raise FileNotFoundError(
            f"best.pkl not found in {args.save_dir}. Ensure main_opt.py saved results with --save_dir."
        )
    ...
    if args.time_sampling != 'video':
        ...
        data = rollout_simulation(
            rng, best_member, s0=None, substrate=substrate, fm=None,
        ...
        with tqdm() as pbar:
            ...
            while args.max_steps is None or steps_done < args.max_steps:
                ...
                while remaining > 0:
                    ...
                    batch_frames = np.asarray(batch_frames[:mb])
                    batch_masses = np.asarray(batch_masses[:mb])
