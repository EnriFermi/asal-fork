...
PROTOCOL_VERSION = "flowlenia-rng-vs-checkpoint-shared-init-300k-v1"
DEFAULT_OUTPUT_ROOT = _REPO_ROOT / (
    "analysis/results/"
    "flowlenia_rng_vs_checkpoint_shared_init_8rng_dup_300k_v1"
)
...
AUDIT_CANDIDATE = "run_000_optimized"

POINT_STEPS = tuple(
    sorted(
        set(range(0, 1_001, 50))
        | set(range(1_250, 10_001, 250))
        | set(range(11_000, HORIZON_STEPS + 1, 1_000))
    )
)
STATE_METRIC_STEPS = tuple(
    sorted(
        set(range(0, 10_001, 50))
        | set(range(11_000, HORIZON_STEPS + 1, 1_000))
    )
)
CHAMFER_HORIZONS = (
...
EMBED_STEPS = tuple(
    sorted(
        ...
        | {
            step
            for offsets in CHAMFER_OFFSETS.values()
            for step in offsets
        }
    )
)
...
def prepare(output_root: Path) -> dict[str, Any]:
    ...
    candidates = [
        dict(row) for row in old_candidates if row["candidate_kind"] == "optimized"
    ]
    if [row["candidate_id"] for row in candidates] != [
        f"run_{idx:03d}_optimized" for idx in range(10)
    ]:
        raise RuntimeError("Expected optimized candidates run_000 through run_009")
    ...
    plan_identity = {
        ...
        "design": {
            "question": (
                "How quickly does continuation-RNG divergence from one exact "
                "initial state reach the divergence between different optimized "
                "checkpoints evaluated at the same absolute simulation step?"
            ),
            "n_optimized_checkpoints": 10,
            ...
            "lanes_per_checkpoint_batch": BATCH_SIZE,
            ...
            "chamfer_horizons": CHAMFER_HORIZONS,
            "chamfer_offsets": CHAMFER_OFFSETS,
        ...
        "metrics": {
            ...
            "between_checkpoints_primary": (
                "distance over all 45 checkpoint pairs using matched shared "
                "initial state and matched unique RNG branch"
            ),
            "between_checkpoints_secondary": (
                "distance over all cross-checkpoint RNG-branch combinations"
            ),
            "harness": (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "FlowLenia init-only continuation-RNG versus optimized-checkpoint "
