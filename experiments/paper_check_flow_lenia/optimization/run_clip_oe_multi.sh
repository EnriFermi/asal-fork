...
: "${PAPER_NUM_OPTIMIZATIONS:=5}"
: "${PAPER_MACHINE_IDX:=0}"
: "${PAPER_NUM_MACHINES:=1}"
: "${PYTHONUNBUFFERED:=1}"
export PAPER_NUM_OPTIMIZATIONS PAPER_MACHINE_IDX PAPER_NUM_MACHINES PYTHONUNBUFFERED
...
if [ -n "${PYTHON_BIN:-}" ]; then
  ...
  conda run --no-capture-output -n "${CONDA_ENV:-onerec}" python "${python_script}" "${config_path}"
else
